...
namespace FleetControl.Application.Commands.Customers.CreateCustomer
{
    public class CreateFleetCustomer_Dto : IHaveCustomMapping
    {
        ...
        {
            configuration.CreateMap<CreateFleetCustomer_Dto, Customer>()
                .ForMember(x => x.Address1, m => m.NullSubstitute(""))

                .ForMember(x => x.LiftingNumber, m => m.NullSubstitute(""))
                .ForMember(x => x.CustomerName, m => m.NullSubstitute(""))
                .ForMember(x => x.EmbossedCustomerName, m => m.NullSubstitute(""))
                .ForMember(x => x.Address2, m => m.NullSubstitute(""))
                .ForMember(x => x.Address1, m => m.NullSubstitute(""))
                .ForMember(x => x.City, m => m.NullSubstitute(""))
                .ForMember(x => x.UsState, m => m.NullSubstitute(""))
                .ForMember(x => x.ZipCode, m => m.NullSubstitute(""))
                .ForMember(x => x.Telephone, m => m.NullSubstitute(""))
                .ForMember(x => x.Contact, m => m.NullSubstitute(""))
                .ForMember(x => x.SalesPerson, m => m.NullSubstitute(""))
                .ForMember(x => x.Address1, m => m.NullSubstitute(""))
